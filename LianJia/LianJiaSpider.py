...
def read_xiaoqu(date):
    ...
    with open(base_file, 'rt') as f:
        for tmp in f:
            ...
            [url, xiaoqu_id, xiaoqu_name, build_age, bulid_type, service_price, service_company, developers, bulid_total, house_total, near_zhongjie, lnglat, avg_price, refer_month, house_type_total, day30_done, rent_num, subway, region, date] = info
            ...
            tmp = (url, xiaoqu_id, xiaoqu_name, build_age, bulid_type, service_price, service_company, developers, bulid_total, house_total, near_zhongjie, lnglat, avg_price, refer_month, house_type_total, day30_done, rent_num, subway, region, date)
            datas.append(tmp)
            if len(datas) == 10:
                try:
...
def get_urls(type):
    """
        ...
        这么处理的原因是解决，链家在展示二手房时仅仅展示前3000套房的限制；
    ...
    with open(base_file, 'rt') as f:
        for tmp in f:
            ...
            if break_flag:
                continue
            info = escape_string(info)
            [url, house_id, title, subtitle, buyer_attention, buyer_see, xiaoqu_name, region, area, ring_road, subway, total, house_type, floor, bulid_area,
            house_structure, use_area, bulid_type, orientation, build_structure, fitment, ladder_rate, warm, lift, property_right_year,
            createtime, property, last_buy_time, house_use, house_limit, property_right_belong, pledge_info, supplement, house_info, date] = info
            # 评分相关
            total_score, defang_score, huxing_score, chuanghu_score, chaoxiang_score, defang_area, defang_rate, defang_price_score, defang_price, fitment_score = score.get_score(house_info, bulid_area, total, fitment)
            ...
            tmp = (url, house_id, title, subtitle, buyer_attention, buyer_see, xiaoqu_name, region, area, ring_road, subway, total,
...
def read_chengjiao(date):
    ...
    with open(base_file, 'rt') as f:
        for tmp in f:
            ...
            tmp = (url, house_id, title, done_price, hangout_price, deal_cycle, adjust_price, buyer_see, buyer_attention, view_count,
            house_type, floor, bulid_area, house_structure, use_area, bulid_type, orientation, bulided_age, fitment,
            build_structure, warm, ladder_rate, property_right_year, lift, lianjia_id, property, hangout_time, house_use,
            house_limit, house_right_belong, date)
            datas.append(tmp)
